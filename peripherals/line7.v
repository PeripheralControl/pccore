//////////////////////////////////////////////////////////////////////////
//
//  File: line7.v;   Resistor Capacitor analog to digital Converter
//
//      This peripheral interfaces a HandAssemble_Pmod Line7 sensor.
//  The idea is that a capacitor is discharged by a pulse from the FPGA.
//  A timer is started for each input.  The timer is stopped when the
//  capacitor charge passes the 0->1 logic threshold of the FPGA pin.
//  A lower timer count indicates a faster charge and so a lower
//  resistance.
//
...
//
//  The HA_Line7 Pmod board has 7 sensors and 8 white LEDs that are
//  controlled by a PWM signal from the FPGA.  This give you the 
//  ability to control the illumination on the floor.  This control
//  can help give more uniform illumination for the sensor since side
//  light has less of an effect.
//
//  Registers
//  0-6:   Sensor values.  8 bits
//  7  :   LED PWM count.  0==off, 255=full on
//  8  :   configuration:
...
module line7(CLK_I,WE_I,TGA_I,STB_I,ADR_I,STALL_O,ACK_O,DAT_I,DAT_O,clocks,pins);
    ...
    inout  [7:0] pins;       // RCC-RC inputs, LED PWM output on pin 7
    ...
    reg    [7:0] pwmval;     // Target LED brightness as a PWM value
    reg    [7:0] pwmcount;   // Continuously counts from 0 to 255
    ...
    begin
        ...
        if (TGA_I & myaddr & WE_I & (ADR_I[3:0] == 4'h7))       // set LED brightness
        begin
            pwmval = DAT_I;
        end
        else if (TGA_I & myaddr & WE_I & (ADR_I[3:0] == 4'h8))  // set config
        begin
    ...
    assign pins[7] = (pwmval <= pwmcount) ? 1 : 0;
    ...
    assign DAT_O = (~myaddr) ? DAT_I : 
                    ...
                    (~TGA_I && data_avail) ? 8'h07 :       // autosend seven bytes
